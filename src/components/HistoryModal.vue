...
const emit = defineEmits<{
  (e: 'close'): void;
  (e: 'clearHistory'): void;
  (e: 'notify', payload: { type: 'success' | 'info' | 'error', title: string, message: string }): void;
}>()

const copiarTexto = async (text: string, tipo: string) => {
  if (!text) return
  try { 
    await navigator.clipboard.writeText(text) 
    emit('notify', { type: 'success', title: 'Copiado!', message: `O texto de ${tipo} foi copiado para a área de transferência.` })
  } catch (err) { 
    console.error('Falha ao copiar:', err) 
    emit('notify', { type: 'error', title: 'Erro', message: 'Não foi possível copiar o texto.' })
  }
}
</script>
...
<template>
  <div v-if="show" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 dark:bg-black/80 backdrop-blur-sm">
    <div class="bg-white dark:bg-[#151822] rounded-3xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden border border-slate-200 dark:border-[#1e2330]">
        ...
        >
          ...
          <div class="flex gap-4 border-t border-slate-200 dark:border-[#1e2330] pt-4" v-if="record.kitsText || record.cerasText">
            <div v-if="record.kitsText" class="flex-1 relative group">
              ...
              <button @click="copiarTexto(record.kitsText, 'Kit\'s')" class="cursor-pointer absolute top-6 right-2 bg-slate-200 dark:bg-[#1e2330] p-1.5 rounded-lg text-slate-600 hover:text-[#ffca28] opacity-0 group-hover:opacity-100 transition shadow-sm">
            ...
            <div v-if="record.cerasText" class="flex-1 relative group">
              ...
              <button @click="copiarTexto(record.cerasText, 'Ceras')" class="cursor-pointer absolute top-6 right-2 bg-slate-200 dark:bg-[#1e2330] p-1.5 rounded-lg text-slate-600 hover:text-[#ffca28] opacity-0 group-hover:opacity-100 transition shadow-sm">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
              </button>
            </div>
          </div>
      ...
      <button v-if="history.length > 0" @click="emit('clearHistory')" class="cursor-pointer w-full py-5 bg-red-50 dark:bg-[#1f1517] hover:bg-red-100 dark:hover:bg-[#2a1b1e] text-red-500 font-bold text-xs tracking-widest uppercase transition">
