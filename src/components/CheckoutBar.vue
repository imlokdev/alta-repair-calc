    ...
    <div class="max-w-7xl mx-auto px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-6 h-auto md:h-24">
      <div class="flex items-center gap-6 w-full md:w-auto">
        <div class="flex flex-col">
          <span class="text-[10px] font-bold tracking-widest text-slate-400 uppercase mb-1">Total a cobrar</span>
        ...
        <div class="flex flex-col max-w-[200px] lg:max-w-[400px]">
          ...
          <span class="text-xs text-slate-500 truncate italic">{{ itemsInOrder }}</span>
        </div>
      </div>
      <div class="flex items-center gap-4 w-full md:w-auto">
        <button @click="$emit('clear')" class="px-8 h-12 rounded-xl font-bold text-xs uppercase tracking-widest text-red-500 border border-red-500 hover:bg-red-500 hover:text-white transition w-full md:w-auto">Limpar</button>
        <button @click="$emit('finalize')" :disabled="grandTotal === 0" class="px-10 h-12 rounded-xl font-black text-xs uppercase tracking-widest text-slate-900 bg-[#ffca28] hover:bg-[#ffd54f] disabled:opacity-50 disabled:cursor-not-allowed transition w-full md:w-auto shadow-lg shadow-yellow-500/20">Finalizar Atendimento</button>
      </div>
    </div>
  </div>
</template>